...
public class Lesk {
	...
	private Set<String> POSSet1 = new HashSet<String>(Arrays.asList("ADJECTIVE", "ADVERB", "NOUN", "VERB"));
	private Set<String> POSSet = new HashSet<String>(Arrays.asList("JJ", "RB", "NN", "VB"));
	...
	private double similarity(ArrayList<String> bag1, ArrayList<String> bag2, String sim_opt) {
		if(bag1.size() == 0 || bag2.size() == 0)return 0.0;
	...
	private ArrayList<String> signatureList(String text, String context_option, String target){
		ArrayList<String> res = new ArrayList<>();
		Annotation document = new Annotation(text);
		pipeline.annotate(document);
		List<CoreMap> sentences = document.get(SentencesAnnotation.class);
		for(CoreMap sentence: sentences) {
			...
			for (CoreLabel token: sentence.get(TokensAnnotation.class)) {
				...
				if(context_option.equals("ALL_WORDS_R")){
					if(!stopwords.contains(word))res.add(word);
				}
				else if(context_option.equals("POS")){
					//System.out.println(word + "," + pos);
					if(pos.length() > 2 && POSSet.contains(pos.substring(0,2))){
						//System.out.println(word + "," + pos);
						res.add(word);
					}
				}
		...
		for(int i = 0; i < ambiguousWords.size(); ++i){
			...
			for(Pair<String, String> target : ambiguousWords.get(i)){
				...
				if(context_option.equals("ALL_WORDS")){
					...
					//System.out.println(Math.max(0, begin) +","+ Math.min(end, sentence.length()));

					for(int loc = Math.max(0, begin); loc < Math.min(end+1, sentence.length()); ++loc){
						context.add(sentence.getWordAt(loc).getLemme());
						//System.out.println("fdsfa " + sentence.getWordAt(loc).getLemme());
					}
				}
				else if(context_option.equals("POS")){
					for(int loc = 0; loc < sentence.length(); ++loc){
						String pos = sentence.getWordAt(loc).getPosTag();
						//System.out.println("fdsfa " + sentence.getWordAt(loc).getLemme() + "," + sentence.getWordAt(loc).getPosTag());
						if(pos.length() < 2 || !POSSet.contains(pos.substring(0,2)))continue;
						context.add(sentence.getWordAt(loc).getLemme());
					}
				}
				count++;
				...
				for(String key : map.keySet()){
					...
					ArrayList<String> bag= signatureList(gloss, context_option, target.getKey());
	...
	private ArrayList<Double> evaluate(ArrayList<String> groundTruths, HashMap<String, Double> predictions, int K) {
		...
		for(String key : top.keySet()){
			if(top.get(key) > 0)overlap.add(key);
		}
		int count = overlap.size();

		overlap.retainAll(truths);

		double precision = (double)overlap.size()/Math.min(count,K);
		if(count == 0)precision = 0;
		double recall = (double)overlap.size()/truths.size();
		if(truths.size() == 0)recall = 0;
		res.add(precision);
		res.add(recall);
		if(res.get(0) == 0 || res.get(1) == 0)res.add(0.0);
		else res.add(2*res.get(0)*res.get(1) / (res.get(0) + res.get(1)));
	...
	public ArrayList<Double> evaluate(int K) {
		...
		for(int i = 0; i < groundTruths.size(); ++i){
			for(int j = 0; j < groundTruths.get(i).size(); ++j){
				...
				precisions.add(tem.get(0));
				recalls.add(tem.get(1));
				f1s.add(tem.get(2));
			}
		}
